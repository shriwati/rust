...
impl ClickHouseConnection {
    ...
    fn validate(host:&str,port:u16,_username:&str,database:&str) -> Result<()> {
        if host.is_empty(){ return Err(anyhow!("Host cannot be empty")); }
        if port == 0 { return Err(anyhow!("Port must be greater than 0")); }
        if database.is_empty() { return Err(anyhow!("Database cannot be empty")); }
        Ok(())
    }
...
pub async fn create_connection(host:&str,port:u16,database:&str,username:&str) -> Result<ClickHouseConnection>
{
